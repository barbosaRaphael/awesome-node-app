...
const errorHandler = (err) => {
    ...
    if (err.message.includes('duplicate key error collection: node-practice.users index: email_1 dup key:')) {
        errors.email = 'The email submited is already registered';
    }
    else if (err.message.includes('duplicate key error collection: node-practice.users index: name_1 dup key:')) {
        errors.name = 'The name provided is already registered';
    }
    // Login Errors
    if (err.message.includes("The email provided is incorrect or isn't registered")) {
